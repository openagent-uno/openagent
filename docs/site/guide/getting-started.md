...
library. The current public product build is **v1.1.0-beta.3**; see the
...
.venv/bin/python -m pip install --pre --find-links . \
  openagent-framework==1.1.0b3 openagent-cli==1.1.0b3
```

The release manifests publish the expected SHA-256 for every first-party wheel:

- [product manifest](https://github.com/openagent-uno/openagent/releases/download/v1.1.0-beta.3/product-release-manifest.json)
