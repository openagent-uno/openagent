            ...
            try:
                ...
                while True:
                    ...
                    if complete:
                        break
                    await asyncio.sleep(0)
                from openagent_core.memory.operational.automation import project_automation_pending
                from openagent_core.memory.operational.search import operational_search_status, sync_operational_search
...
async def claim_created_resource(request: web.Request, resource_type: str, resource_id: str) -> None:
    ...
    if db is None:
        return
    conn = await db._ensure_connected()
    from openagent_core.memory.operational.automation import claim_resource
...
async def handle_session_messages(request: web.Request) -> web.Response:
    try:
        ...
        from openagent_core.memory.artifacts import attachment_refs_for_messages_on_connection
...
def _search_target(row: dict[str, Any]) -> dict[str, Any]:
    from openagent_core.memory.operational.service import search_target
...
async def _search_row_visible(conn: Any, row: dict[str, Any], access: AccessContext) -> bool:
    from openagent_core.memory.operational.service import search_row_visible
...
async def _granted_search_resources(
    ...
    from openagent_core.memory.operational.service import granted_search_resources
...
def _normalize_public_status(raw: Any) -> tuple[str, bool]:
    try:
        from openagent_core.memory.operational.enums import normalize_run_status
...
def _redacted_summary(value: Any) -> str | None:
    if value is None:
        return None
    from openagent_core.memory.operational.search import redact_search_text
