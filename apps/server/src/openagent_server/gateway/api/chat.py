...
Wraps a :class:`~openagent_core.stream.session.StreamSession` +
:class:`~openagent_core.stream.channel.BatchedChannel` so any HTTP client
...
async def _get_or_create_session(
    ...
    async with _sessions_registry_lock:
        ...
        if collaboration is not None and collaboration.owns(session_id):
            from openagent_server.gateway.collaboration import BusyError
