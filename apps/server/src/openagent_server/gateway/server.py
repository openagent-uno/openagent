...
from openagent_server.bootstrap import start_stream
...
from openagent_server.gateway import protocol as P
from openagent_server.gateway.commands import command_help_text
from openagent_server.gateway.sessions import SessionManager
from openagent_server.gateway.terminals import TerminalManager
from openagent_server.gateway.capabilities import (
...
# extension sets in ``openagent_core.mcp.servers.attachments.adapters``.
...
class Gateway:
    ...
    def _prepare_iroh_site(self) -> None:
        ...
        from openagent_identity.iroh_node import NetworkAlpn as _Alpn
    ...
    async def _broadcast_child_frame(self, frame: dict) -> None:
        ...
        from openagent_core.stream.child_stream import child_frame_to_event
        from openagent_client_transport.stream.wire import event_to_wire
    ...
    async def start(self) -> None:
        ...
        from openagent_identity.transport.agent_iroh_site import AgentSite
    ...
    async def _maybe_start_webhook_listener(self) -> None:
        ...
        agent down."""
        from openagent_server.gateway.webhook_site import WebhookSite
        try:
    ...
    async def _handle_upload(self, request):
        ...
        from openagent_server.gateway.api.artifacts import _read_form
        ...
        from openagent_core.voice.voice import is_audio_file
        ...
        if is_audio_file(filename):
            ...
            try:
                from openagent_core.voice.voice import transcribe
    ...
    async def _handle_capabilities_ws(self, request):
        ...
        from openagent_identity.transport.aiohttp_iroh_site import (
            ...
            async for msg in ws:
                ...
                if isinstance(t, str) and t.startswith("ui_"):
                    try:
                        ...
                        from openagent_dashboards.repository import (
                            CustomViewConflict,
                            CustomViewError,
    ...
    async def _handle_command(
        ...
        *, handle: str | None = None, arg: str | None = None,
        ...
        if name in ("new", "reset", "clear"):
            ...
            from openagent_server.gateway.api.usage import _usage_summary_for_agent
            ...
            if not session_id:
                ...
                try:
                    from openagent_core.core.context_report import (
            ...
            if not session_id:
                ...
                try:
                    ...
                    from openagent_client_transport.stream.wire import event_to_wire as _compact_e2w
                    ...
                    if result is None:
                        ...
                        try:
                            ...
                            from openagent_core.stream.events import ContextReport as _CR
    ...
    async def _handle_terminal_frame(
        ...
        *,
        ...
        if t == P.TERMINAL_OPEN:
            ...
            from openagent_server.gateway.terminals import PTY_SUPPORTED
    ...
    async def _handle_stream_frame(
        ...
        *, handle: str | None = None,
        ...
        from openagent_core.stream.events import Attachment, SessionClose, SessionOpen, TextFinal
    ...
    async def _interrupt_stream_sessions(
        ...
        *,
        ...
        """
        from openagent_core.stream.events import Interrupt, now_ms
