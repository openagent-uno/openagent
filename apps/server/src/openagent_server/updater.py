...
def check_for_update(channel: str | None = None) -> UpdateInfo | None:
    ...
    try:
        from openagent_server.update_guard import rolled_back_versions
...
def apply_update(new_exe: Path) -> None:
    ...
    """
    from openagent_server._frozen import executable_path
...
def verify_new_binary(new_exe: Path, expected_version: str | None = None) -> None:
    ...
    (:mod:`openagent_server.update_guard`) catches "starts here but unhealthy under
...
def perform_self_update_sync(channel: str | None = None) -> tuple[str, str]:
    ...
    # successful update.
    try:
        from openagent_server.update_guard import record_pending
