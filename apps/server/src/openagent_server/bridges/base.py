...
from openagent_core.media import (
    Attachment,
...
from openagent_client_transport.stream.wire import SESSION_COMPACTED, event_to_wire, wire_to_event
...
def format_tool_status(raw: str) -> str:
    ...
    :mod:`openagent_server.channels.tool_labels`, the twin of the app's tool chips).
    """
    from openagent_core.media import parse_status_event
    from openagent_server.channels.tool_labels import status_line
...
def format_tool_message(raw: str) -> str | None:
    ...
    * tool *completion* pings — the runtime emits a (running, done) pair
    ...
    tool chips (see :mod:`openagent_server.channels.tool_labels`). Mirrors the Hermes
    UX of narrating each tool call in-chat.
    """
    from openagent_core.media import parse_status_event
    from openagent_server.channels.tool_labels import message_line
    evt = parse_status_event(raw)
    if evt is None:
        return None
...
class BaseBridge:
    ...
    async def dispatch_turn(
        ...
        *,
        ...
        try:
            from openagent_core.core.hooks import expand_quick_command
        ...
        try:
            from openagent_core.core.hooks import fire as _hook_fire
            _hook_fire(
                "turn_end",
                bridge=self.name,
                session_id=session_id,
    ...
    async def transcribe_with_fallback(self, file_path: str) -> str:
        ...
        """
        from pathlib import Path as _Path
        from openagent_core.voice.voice import transcribe as transcribe_local
