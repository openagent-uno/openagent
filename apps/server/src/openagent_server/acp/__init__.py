...
subcommand (:mod:`openagent_server.cli`).
...
(:mod:`openagent_server.acp.agent`, :mod:`openagent_server.acp.events`) is imported ONLY when the
``openagent acp`` subcommand actually runs. Importing this package (``openagent_server.acp``)
...
* ``session/new`` → mint a session id and hold a
  :class:`~openagent_core.stream.session.StreamSession` (``profile="batched"``), the same
...
# here — that is what keeps ``import openagent_server.acp`` safe without the optional extra.
