...
:class:`~openagent_core.stream.session.StreamSession` in ``profile="batched"`` built on
the process's single OpenAgent :class:`~openagent_core.core.agent.Agent`. Each ACP
...
the session's inbound bus (like :class:`~openagent_core.stream.channel.BatchedChannel`)
and streams the resulting ``session.outbound`` events out as ACP
``session_update`` notifications via :func:`openagent_server.acp.events.stream_turn`.
...
from openagent_server.acp.events import stream_turn
...
class OpenAgentACPAgent(acp.Agent):
    """ACP agent surface over a single OpenAgent :class:`~openagent_core.core.agent.Agent`."""
