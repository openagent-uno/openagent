...
from openagent_identity.cli_commands import network_group
...
async def _repack_serve_invites_after_start(
    *,
    ...
    from openagent_identity.cli_commands import (
...
def _cleanup_stale_openagent_frozen_extract_dirs(
    ...
    """
    from openagent_server._frozen import bundle_dir, is_frozen
...
def _startup_cleanup() -> None:
    ...
    from openagent_server._frozen import (
        is_frozen,
    ...
    # Must happen BEFORE the first ``import openagent_server.mcp._runtime.function``
    ...
    # (:mod:`openagent_server.update_guard`) restores when a freshly-installed binary
...
@click.version_option(
    version=__import__("openagent_server").__version__,
    prog_name="openagent",
...
def main(ctx, config: str, agent_dir: str | None, verbose: bool):
    ...
    if ctx.invoked_subcommand == "serve":
        try:
            from openagent_server.update_guard import boot_guard
...
def config_cmd(ctx, as_json: bool, show_env: bool) -> None:
    ...
    from openagent_core.core import paths as _paths
...
def selfcheck(quiet: bool, expect: str | None) -> None:
    ...
    """
    import openagent_server as _src
...
def update(ctx, yes: bool, no_restart: bool) -> None:
    ...
    from openagent_server.server import get_installed_version, run_upgrade
    from openagent_server.setup.installer import restart_service
    ...
    async def _serve():
        ...
        try:
            async with server:
                ...
                try:
                    from openagent_server.update_guard import mark_healthy
                    mark_healthy()
...
def acp(ctx, agent_dir: str | None):
    ...
    # ``openagent_server.cli`` never pulls it in. A missing extra is a clean, actionable
    ...
    from openagent_server.server import _build_agent
    ...
    async def _run_acp():
        ...
        from openagent_server.acp.agent import OpenAgentACPAgent
...
def mcp_server_cmd(name: str):
    ...
    """
    if name == "scheduler":
        from openagent_core.mcp.servers.scheduler.server import main as scheduler_main
        scheduler_main()
        return
    if name == "mcp-manager":
        from openagent_core.mcp.servers.mcp_manager.server import main as mcp_manager_main
        mcp_manager_main()
        return
    if name == "model-manager":
        from openagent_core.mcp.servers.model_manager.server import main as model_manager_main
        model_manager_main()
        return
    if name == "workflow-manager":
        from openagent_core.mcp.servers.workflow_manager.server import main as workflow_manager_main
        workflow_manager_main()
        return
    if name == "events-manager":
        from openagent_core.mcp.servers.events_manager.server import main as events_manager_main
        events_manager_main()
        return
    if name == "budget-manager":
        from openagent_core.mcp.servers.budget_manager.server import main as budget_manager_main
        budget_manager_main()
        return
    if name == "media-gen":
        from openagent_core.mcp.servers.media_gen.server import main as media_gen_main
        media_gen_main()
        return
    if name == "memory-search":
        from openagent_core.mcp.servers.memory_search.server import main as memory_search_main
        memory_search_main()
        return
    if name == "skill-data":
        from openagent_core.mcp.servers.skill_data.server import main as skill_data_main
        skill_data_main()
        return
...
from openagent_core.memory.vault.cli import vault_group  # noqa: E402
...
def cmd_top_level_invite(ctx, handle, role, ttl, uses):
    ...
    from openagent_identity.cli_commands import _run_invite
...
def doctor(ctx, fix: bool) -> None:
    ...
    from openagent_server.setup.bootstrap import run_doctor as _run_doctor

    if fix:
        from openagent_server.setup.bootstrap import ensure_git as _ensure_git
