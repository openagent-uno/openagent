...
from openagent_core.core.logging import clear as clear_event_log, elog
...
# and the next ``from openagent_server._frozen import is_frozen`` in run_upgrade
...
import openagent_server._frozen  # noqa: F401 — preload for concurrent-update safety
import openagent_server.updater  # noqa: F401 — preload for concurrent-update safety
import openagent_server.update_guard  # noqa: F401 — preload for concurrent-update safety
...
try:
    _INITIAL_EXECUTABLE_MTIME: float | None = (
        openagent_server._frozen.executable_path().stat().st_mtime
        if openagent_server._frozen.is_frozen()
        else None
    )
...
def _build_agent(config: dict) -> Agent:
    ...
    # ``openagent_core.core.safety.check_command_allowed`` at the ``shell_exec`` callsite
    ...
    if _sched_cfg.get("timezone"):
        from openagent_core.memory.schedule import DEFAULT_TZ_ENV, validate_timezone
    ...
    # "break my agent", and will pick off. See ``openagent_core.core.safety._compile_allow``.
    ...
    # ``openagent_core.mcp.install_policy.check_mcp_install_allowed`` at the marketplace
    ...
    # ``openagent_identity.auth.peer_policy`` at the single agent-ALPN branch of the
    ...
    # derived artifacts, dream-mode maintenance). See ``openagent_core.memory.vault``.
...
def _build_bridges(config: dict, per_bridge_url: dict[str, str]) -> list:
    ...
    for name, cfg in channels_config.items():
        ...
        if name == "telegram":
            ...
            from openagent_server.bridges.discord import DiscordBridge
            ...
            from openagent_server.bridges.whatsapp import WhatsAppBridge
            ...
            from openagent_server.bridges.slack import SlackBridge
...
class AgentServer:
    ...
    async def _build_network_state(self):
        ...
        """
        from openagent_identity.state import NetworkState, StandaloneAgentError
    ...
    async def start(self) -> None:
        ...
        from openagent_identity.transport.anyio_cancel_guard import _patch_deliver_cancellation
    ...
    async def _start_product_services(self, local_e2e):
        ...
        try:
            from openagent_core.learning.curator import start as _curator_start
        ...
        try:
            from openagent_core.memory.vault.autocommit import start as _vault_autocommit_start
        ...
        try:
            from openagent_core.memory.semantic_index_builder import start as _sem_index_start
        ...
        try:
            from openagent_core.core.quality_digest import start as _quality_digest_start
    ...
    async def _build_bridge_session_and_bridges(self) -> None:
        ...
        """
        from openagent_core.core.paths import get_agent_dir
        from openagent_identity.bridge_session import (
            BridgeSession,
    ...
    async def _stop(self, timeout: float = 15) -> None:
        ...
        try:
            ...
            pass
        try:
            from openagent_core.memory.vault.service import close_all as _vault_close_all
    ...
    async def _sync_quality_scorer(self, scheduler) -> None:
        ...
        """
        from openagent_core.core.config import self_improvement_settings
...
def _read_disk_binary_version() -> str | None:
    ...
    import subprocess
    try:
        path = openagent_server._frozen.executable_path()
...
def run_upgrade(channel: str | None = None) -> tuple[str, str]:
    ...
    if is_frozen():
        if _binary_replaced_by_sibling():
            ...
            return current, new
        from openagent_server.updater import perform_self_update_sync
...
async def _do_auto_update(
    ...
    if mode == "auto":
        ...
        if gateway is not None:
            from openagent_server.gateway.api.control import request_restart
