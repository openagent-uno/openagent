...
def _score(
    ...
    if not case.skip_policy_route:
        ...
        from openagent_support.local_support_controller import _TENANTS, _DEFAULT_TENANT
...
async def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="openagent-support-ops-") as tmp:
        ...
        # The support integration is independently installable.  The product
        # server is needed only by this full operational entrypoint, not when
        # importing corpus helpers from the support wheel's regression suite.
                ...
                async with limiter:
                        ...
                        try:
                            with lean_local_event_scope(True), dry_run_scope(True):
                                if args.controller:
                                    from openagent_support import local_support_controller
