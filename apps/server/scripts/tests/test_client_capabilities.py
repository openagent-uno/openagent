...
async def t_artifact_chunks(ctx: TestContext) -> None:
    ...
    from openagent_server.gateway.capabilities import MAX_ARTIFACT_BYTES_PER_CALL
    ...
    from openagent_server.gateway.capabilities import MAX_ARTIFACT_TRANSFERS_PER_CALL
...
async def t_scoped_tool_search(ctx: TestContext) -> None:
    ...
    from openagent_server.gateway.capabilities import CapabilityRegistry
...
async def t_mutating_cancellation_audit(ctx: TestContext) -> None:
    import openagent_server.gateway.capabilities as capability_module
    from openagent_server.gateway.capabilities import CapabilityRegistry
...
async def t_runtime_mcp_deep_envelope_limits(ctx: TestContext) -> None:
    import openagent_server.mcp.servers.tool_search.adapters as adapters
...
async def t_cross_language_argument_hash(ctx: TestContext) -> None:
    from openagent_server.gateway.capabilities import _canonical_json_sha256
...
async def t_live_device_revocation(ctx: TestContext) -> None:
    ...
    from openagent_identity.auth.middleware import NetworkAuthState
...
async def t_chat_attach_epoch_recheck(ctx: TestContext) -> None:
    ...
    from openagent_server.gateway.server import Gateway, _StreamHolder
    from openagent_core.stream.events import TextFinal
...
async def t_automation_origin_clearing(ctx: TestContext) -> None:
    import openagent_server.core.scheduler as scheduler_module
...
async def t_stream_turn_security_context(ctx: TestContext) -> None:
    import openagent_server.memory.artifacts as artifact_module
    import openagent_server.stream.content_parts as content_parts_module
...
async def t_wire_and_prompt_tail(ctx: TestContext) -> None:
    ...
    from openagent_server.gateway.capabilities import CAPABILITY_PROTOCOL
