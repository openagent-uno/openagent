...
async def t_inproc_roundtrip(ctx: TestContext) -> None:
    from openagent_identity.transport.inproc import (
        InProcConnection,
        InProcDialer,
    )
...
async def t_inproc_close_wakes_accept(ctx: TestContext) -> None:
    from openagent_identity.transport.inproc import InProcConnection
...
async def t_bridge_rejects_bad_name(ctx: TestContext) -> None:
    from openagent_identity.bridge_session import BridgeSession
...
async def t_iroh_site_stream_lifetime_not_capped(ctx: TestContext) -> None:
    import openagent_identity.transport.aiohttp_iroh_site as iroh_site
