...
async def t_telegram_album_group_session(ctx: TestContext) -> None:
    from openagent_server.bridges.telegram import TelegramBridge, _Extracted
...
async def t_telegram_post_download_limit_and_staging(ctx: TestContext) -> None:
    import openagent_server.bridges.telegram as telegram_mod
    from openagent_server.bridges.telegram import TelegramBridge
...
async def t_slack_download_outcome_and_staging(ctx: TestContext) -> None:
    import openagent_server.bridges.slack as slack_mod
    from openagent_server.bridges.slack import SlackBridge
...
async def t_slack_handlers_scope_and_notifications(ctx: TestContext) -> None:
    from openagent_server.bridges.slack import SlackBridge, _SlackDownloadOutcome
...
async def t_whatsapp_download_ssrf_guards(ctx: TestContext) -> None:
    import aiohttp
    from openagent_server.bridges.whatsapp import (
        WhatsAppBridge,
