...
subprocess-leak test spawns ``python -m openagent_server.cli --help`` (which runs
...
async def t_tqdm_threading_lock(ctx: TestContext) -> None:
    ...
    import threading
    import openagent_server.cli  # noqa: F401  — module-level side effect: pin tqdm
...
async def t_subprocess_no_semaphore_leak(ctx: TestContext) -> None:
    """End-to-end leak check: spawn ``python -m openagent_server.cli --help`` (the
    ...
    proc = subprocess.run(
        [str(python), "-m", "openagent_server.cli", "--help"],
        cwd=str(repo_root),
        capture_output=True,
        text=True,
        timeout=30,
