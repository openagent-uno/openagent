...
``openagent_server._frozen.is_frozen`` → True and ``executable_path`` → a sandbox
...
def _patches(target: Path):
    return (
        patch("openagent_server._frozen.is_frozen", return_value=True),
        patch("openagent_server._frozen.executable_path", return_value=target),
    )
...
async def t_record_pending(ctx: TestContext) -> None:
    import json
    import tempfile
    import openagent_server.update_guard as g
...
async def t_guard_noop_unfrozen(ctx: TestContext) -> None:
    ...
    with patch("openagent_server._frozen.is_frozen", return_value=False):
        assert g.boot_guard() is None
