...
async def t_updater_callable(ctx: TestContext) -> None:
    ...
    from openagent_server.updater import check_for_update, UpdateInfo, perform_self_update_sync
    assert openagent_server.__version__ and isinstance(openagent_server.__version__, str)
...
async def t_updater_prefers_server_asset(ctx: TestContext) -> None:
    import openagent_server
    import openagent_server.updater as updater
...
async def t_apply_update_bundle_swap(ctx: TestContext) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch("openagent_server._frozen.executable_path", return_value=cur_bin), \
...
async def t_updater_logs_bad_tag(ctx: TestContext) -> None:
    ...
    upd_logger = logging.getLogger("openagent_server.updater")
...
async def t_apply_update_rollback_bare(ctx: TestContext) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with (
            patch("openagent_server._frozen.executable_path", return_value=cur),
...
async def t_try_elog_safe(ctx: TestContext) -> None:
    ...
    import openagent_server.core.logging as logmod
...
async def t_run_upgrade_sibling_swap(ctx: TestContext) -> None:
    ...
    """
    import openagent_server.server as server_mod
    ...
    with patch.object(server_mod, "_INITIAL_EXECUTABLE_MTIME", 1000.0), \
         patch.object(server_mod.openagent_server._frozen, "is_frozen", return_value=True), \
         patch.object(server_mod.openagent_server._frozen, "executable_path", return_value=_FakePath()), \
         patch.object(server_mod, "_read_disk_binary_version", return_value="0.12.99"), \
         patch("openagent_server.updater.perform_self_update_sync", side_effect=fake_perform_self_update_sync):
...
async def t_selfcheck_loads_operational_resources(ctx: TestContext) -> None:
    ...
    from openagent_server.cli import main

    result = CliRunner().invoke(
        main,
        ["selfcheck", "--quiet", "--expect", openagent_server.__version__],
    )
    assert result.exit_code == 0, result.output
    assert result.output.strip() == openagent_server.__version__
...
async def t_perform_self_update_cleans_temp(ctx: TestContext) -> None:
    ...
    with (
        ...
        _patch("openagent_server.update_guard.record_pending", side_effect=lambda *a, **k: recorded.update(a=a, k=k)),
    ):
...
async def t_updater_skips_rolled_back(ctx: TestContext) -> None:
    ...
    with (
        ...
        patch.object(updater, "urlopen",
                     return_value=_FakeHTTPResponse(json.dumps(payload).encode())),
        patch("openagent_server.update_guard.rolled_back_versions", return_value={"9.9.9"}),
    ):
        info = updater.check_for_update()
...
async def t_updater_captures_digest(ctx: TestContext) -> None:
    ...
    with (
        ...
        patch.object(updater, "urlopen",
                     return_value=_FakeHTTPResponse(json.dumps(payload).encode())),
        patch("openagent_server.update_guard.rolled_back_versions", return_value=set()),
    ):
        info = updater.check_for_update()
    assert info is not None
