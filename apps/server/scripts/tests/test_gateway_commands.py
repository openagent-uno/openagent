...
async def t_stop_scoped_preserves_others(ctx: TestContext) -> None:
    ...
    """
    from openagent_server.gateway.sessions import _QueuedItem
...
async def _attach_live_turn(h: "_Harness", client: str, raw_sid: str):
    ...
    from openagent_server.gateway.server import _StreamHolder
...
async def t_commands_api(ctx: TestContext) -> None:
    import json
    from openagent_server.gateway.commands import COMMAND_MAP
    from openagent_server.gateway.api.commands import handle_list
