...
async def t_telegram_import(ctx: TestContext) -> None:
    import openagent_server.bridges.telegram as mod  # noqa: F401
...
async def t_discord_import(ctx: TestContext) -> None:
    import openagent_server.bridges.discord as mod  # noqa: F401
    has_class = any(inspect.isclass(obj) for _, obj in inspect.getmembers(mod))
...
async def t_whatsapp_import(ctx: TestContext) -> None:
    import openagent_server.bridges.whatsapp as mod  # noqa: F401
    has_class = any(inspect.isclass(obj) for _, obj in inspect.getmembers(mod))
...
async def t_bridge_base(ctx: TestContext) -> None:
    from openagent_server.bridges.base import BaseBridge, format_tool_status
...
async def t_bridge_listener_exit_marks_gateway_lost(ctx: TestContext) -> None:
    import asyncio
    from openagent_server.bridges.base import BaseBridge
...
async def t_bridge_gateway_reconnect_delay_caps(ctx: TestContext) -> None:
    import openagent_server.bridges.base as bridge_mod
    from openagent_server.bridges.base import BaseBridge
...
async def t_bridges_use_shared_dispatch(ctx: TestContext) -> None:
    ...
    import openagent_server.bridges.telegram as tg
    import openagent_server.bridges.discord as dc
    import openagent_server.bridges.whatsapp as wa
...
async def t_telegram_seen_set_bounded(ctx: TestContext) -> None:
    ...
    from openagent_server.bridges.telegram import _SEEN_UPDATE_IDS_MAX
...
async def t_whatsapp_no_placeholder(ctx: TestContext) -> None:
    ...
    from openagent_server.bridges import whatsapp as wa_mod
...
async def t_no_hourglass_placeholder_anywhere(ctx: TestContext) -> None:
    ...
    import openagent_server.bridges.discord as dc
    import openagent_server.bridges.whatsapp as wa
    import openagent_server.bridges.slack as sl
    import openagent_server.bridges.base as base
...
async def t_telegram_send_attachment_dispatch(ctx: TestContext) -> None:
    ...
    from pathlib import Path
    from openagent_server.bridges.telegram import TelegramBridge
    from openagent_core.media import Attachment
...
async def t_format_tool_message(ctx: TestContext) -> None:
    from openagent_server.bridges.base import format_tool_message
...
async def t_friendly_memory_labels(ctx: TestContext) -> None:
    import json
    from openagent_server.bridges.base import format_tool_message, format_tool_status
    from openagent_core.media import parse_status_event
    from openagent_server.channels.tool_labels import tool_label
