...
async def t_dream_prompt(ctx: TestContext) -> None:
    from openagent_server.server import DREAM_MODE_PROMPT, DREAM_MODE_TASK_NAME
...
async def t_dream_uses_logs_mcp(ctx: TestContext) -> None:
    ...
    """
    from openagent_server.server import DREAM_MODE_PROMPT
...
async def t_no_manager_review(ctx: TestContext) -> None:
    import openagent_server.server as server
    import openagent_server.core.builtin_tasks as bt
