...
from openagent_identity.identity import (
    Identity,
...
async def mint_first_user_invite(
    *,
    ...
    """
    from openagent_identity.coordinator_addr_cache import read_cache
...
def _node_id_for(identity: Identity) -> str:
    ...
    encoding.
    """
    from openagent_identity.iroh_node import _node_id_from_secret
...
def cmd_loopback(ctx, target, handle_override, agent_handle, password_stdin, print_port):
    ...
    """
    from openagent_identity.iroh_node import DialError
...
async def _run_loopback(ctx, target, handle_override, agent_handle, password_stdin, print_port):
    ...
    from openagent_identity import user_store
    ...
    from openagent_identity.ticket import InviteTicket, TicketError, looks_like_ticket
...
async def _run_join(ctx, ticket_str: str, handle_override: str | None, label: str | None):
    from openagent_identity.ticket import InviteTicket, TicketError
    from openagent_identity.client.login import agent_login
    from openagent_identity.peers import PeerStore, coordinator_node_id_to_pubkey_bytes
    from openagent_identity.auth.device_cert import verify_cert, CertVerificationError
...
async def _run_peer_chat(ctx, network_id_or_name: str, message: str,
    ...
    from openagent_identity.peers import PeerStore, make_dialer_for_peer, coordinator_node_id_to_pubkey_bytes
    ...
    try:
        ...
        try:
            from openagent_identity.client.login import LoginError
