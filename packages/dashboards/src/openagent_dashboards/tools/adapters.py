...
The App registers these tools for its authenticated originating turn. The
product service shares the persistent data runtime with the gateway; service
availability never registers these tools in the global catalog.  The model can
...
from openagent_dashboards.compiler import (
    COMPONENTS,
    MAX_DEPTH,
    MAX_NODES,
    MAX_SOURCE_BYTES,
...
from openagent_dashboards.bundles import (
    MAX_ASSET_BYTES,
    MAX_BUNDLE_BYTES,
    MAX_BUNDLE_FILES,
    MAX_SCRIPT_BYTES,
...
def build_runtime_toolkit(pool: Any) -> Any:
    ...
    async def access_for_turn() -> AccessContext:
        ...
        raise PermissionError(
            "Dashboard tools require the authenticated originating App capability"
        )
